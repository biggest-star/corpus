...
<form class="cart" action="<?php echo esc_url( apply_filters( 'woocommerce_add_to_cart_form_action', $product->get_permalink() ) ); ?>" method="post" enctype='multipart/form-data'>
		...
		<tbody>
			...
			if ( version_compare( WC_VERSION, '3.0', '<' ) ) {
						...
						<td class="label">
							...
							<?php
								if ( $product->is_visible() ) {
									echo '<a class="eut-subtitle eut-text-content eut-text-hover-primary-1" href="' . esc_url( apply_filters( 'woocommerce_grouped_product_list_link', get_permalink(), $product_id ) ) . '">' . esc_html( get_the_title() ) . '</a>';
								} else {
				...
				foreach ( $grouped_products as $grouped_product ) {
					...
					echo '<tr id="product-' . esc_attr( get_the_ID() ) . '" class="eut-border woocommerce-grouped-product-list-item ' . esc_attr( implode( ' ', get_post_class() ) ) . '">';
					...
					foreach ( $grouped_product_columns as $column_id ) {
						...
						switch ( $column_id ) {
							...
							case 'label':
								...
								$value .= $grouped_product->is_visible() ? '<a class="eut-subtitle eut-text-content eut-text-hover-primary-1" href="' . esc_url( apply_filters( 'woocommerce_grouped_product_list_link', get_permalink( $grouped_product->get_id() ), $grouped_product->get_id() ) ) . '">' . $grouped_product->get_name() . '</a>' : $grouped_product->get_name();
								$value .= '</label>';
								break;
							case 'price':
	...
	<?php if ( $quantites_required ) : ?>
		...
		<button type="submit" class="eut-btn eut-btn-alt eut-black-color"><?php echo wp_kses_post( $product->single_add_to_cart_text() ); ?></button>
