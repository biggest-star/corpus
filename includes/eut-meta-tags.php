...
if ( !function_exists('eut_opengraph_add_prefix') ) {
	function eut_opengraph_add_prefix( $output ) {

		//if ( eut_visibility( 'meta_opengraph' ) ) {
      if(true){
			$prefixes = array(
				'og' => 'http://ogp.me/ns#',
			);
			//$prefixes = apply_filters( 'eut_opengraph_prefixes', $prefixes );
...
add_filter( 'language_attributes', 'eut_opengraph_add_prefix' );

if ( !function_exists('eut_print_meta_tags') ) {
	function eut_print_meta_tags() {
		...
		if ( eut_visibility( 'meta_viewport_responsive', '1' ) ) {
		...
		if ( eut_visibility( 'meta_opengraph' ) || eut_visibility( 'meta_twitter' ) ) {
			...
			if ( is_singular() || is_front_page() || is_home() ) {
				...
				//Twitter Cards
				if ( eut_visibility( 'meta_twitter' ) ) {
		...
		$meta_name_tags = apply_filters( 'eut_meta_name_tags', $meta_name_tags );
		$meta_property_tags = apply_filters( 'eut_meta_property_tags', $meta_property_tags );
...
add_action( 'wp_head', 'eut_print_meta_tags', 5 );
