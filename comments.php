...
<?php if ( have_comments() ) : ?>
	...
	<div id="eut-comments" class="eut-section">
		...
		<ul>
		<?php wp_list_comments( 'type=comment&callback=eut_comments' ); ?>
		</ul>
	</div>
	<!-- End Comments -->
...
<?php
		...
		$args = array(
			...
			'logged_in_as' =>
				'<p class="logged-in-as">' .  esc_html__('Logged in as','corpus') .
				'<a href="' . admin_url( 'profile.php' ) . '"> ' . $user_identity . '</a>. ' .
				'<a href="' . wp_logout_url( get_permalink() ) . '" title="' . esc_attr__( 'Log out of this account', 'corpus' ) . '"> ' . esc_html__( 'Log out', 'corpus' ) . '</a></p>',
